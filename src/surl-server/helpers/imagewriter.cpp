...
Imagewriter::Imagewriter(Bit16u dpi, Bit16u paperSize, Bit16u bannerSize, char* output, bool multipageOutput, int charset)
{
#ifdef HAVE_SDL
...
extern "C" void imagewriter_init(int pdpi, int ppaper, int banner, char* poutput, bool mpage, int charset)
{
	if (defaultImagewriter != NULL) return; //if Imagewriter on this port is initialized, reuse it
	defaultImagewriter = new Imagewriter(pdpi, ppaper, banner, poutput, mpage, charset);
}
