...
static status *is_root_status_at_top(list *l) {
  ...
  root_status = NULL;
  full = (l->root && !strcmp(l->root, l->ids[l->first_displayed_post]));
  if (full) {
...
void cli(void) {
  char cur_list = 0, to_clear, to_show;
  list **l, *prev_list;
  ...
  while (cur_action != QUIT) {
    switch(cur_action) {
      ...
      case BACK:
        if (cur_list > 0) {
          if (l[cur_list]->first_displayed_post == 0 && !l[cur_list]->account)
