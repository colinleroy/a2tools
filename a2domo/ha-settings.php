<?php
  ...
  $list = [
    ...
    "switches" => [
      ...
      "body" => json_encode(["template" => "{%set switches = states|selectattr('entity_id', 'search', '^switch\.') %}
...
{% endfor %}"])
    ...
    "climate" => [
      "token_header" => "Authorization: Bearer $token",
      "content_type" => "application/json",
      "caps" => "", /* comma-separated list, can contain CAN_AWAY and CAN_SCHEDULE */
      "full_home_zone" => "", /* optionally specify zone id for full home */
      "hot_water_zone" => "", /* optionally specify zone id for hot water setting */
      "endpoint" => "$ha_root_url/api/template",
      "body" => json_encode(["template" => "{%set zones = states|selectattr('entity_id', 'search', '^climate\.') %}
{% for item in zones -%}
{{item.entity_id}};{{item.attributes.friendly_name}};{{ item.attributes.temperature|default('0') }};{{ item.attributes.current_temperature|default('0') }};{{ item.attributes.current_humidity|default('0') }}
{% endfor %}"])
    ],
...
{% endfor %}"])
  ...
  $control = [
    "switches" => [
      "token_header" => "Authorization: Bearer $token",
      "content_type" => "application/json",
      "endpoint" => "$ha_root_url/api/services/switch/toggle",
      "body" => '{"entity_id": "{SWITCH_ID}" }'
    ],

    "climate" => [
      "token_header" => "Authorization: Bearer $token",
      "content_type" => "application/json",
      "endpoint" => "$ha_root_url/api/services/climate/set_temperature",
      "body" => '{"entity_id": "{ZONE_ID}", "temperature": "{SET_TEMP}" }'
    ],
