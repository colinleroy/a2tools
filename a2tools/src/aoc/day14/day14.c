...
static void update_sand(int prev_x, int prev_y, int x, int y) {
#ifdef __CC65__
  tgi_setcolor(TGI_COLOR_BLACK);
  tgi_setpixel(OFF_X(prev_x), OFF_Y(prev_y));
  if (x > 0 && y > 0) {
    tgi_setcolor(TGI_COLOR_ORANGE);
